...
pub struct AgentInfo {
    ...
    pub template: String,
    pub status: String,
    pub uptime: String,  // Pre-formatted string like "1d 1h"
}

/// A Claude Code subagent task
#[derive(Debug, Clone)]
...
pub struct App {
    ...
    pub claude_tasks: Vec<ClaudeTaskInfo>,
...
impl App {
    ...
    pub fn refresh(&mut self) {
        ...
        self.claude_tasks = data::fetch_claude_tasks();

        // Fetch real events from journal
        let journal_events = data::fetch_events(&self.paths, 50);
        if !journal_events.is_empty() {
            self.events = journal_events;
        }
    }
